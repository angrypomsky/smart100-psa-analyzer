...
In Level 1 PSA, each TH simulation scenario must be converted into a judgment of success or failure of safety functions in order to quantify the core damage frequency (CDF) [1]. However, a standard event tree has a structural limitation in that it classifies each safety function only as a binary success or failure. SMART (System-integrated Modular Advanced ReacTor), the reactor design used for verification in this study, is a 100 MWth integral pressurized water reactor developed by KAERI, equipped with a four-train Passive Residual Heat Removal System (PRHRS) as its key safety system [7]. For a multi-train system such as PRHRS, the number of operating trains (0 to 4) has a direct effect on accident outcomes, yet compressing this into a binary representation discards information about partial system performance.

A Dynamic Event Tree (DET) structure is needed to address this limitation. A DET retains the discrete number of operating safety-system trains as a branching variable, enabling exploration of how marginal changes in system availability affect the PCT distribution — sensitivity that cannot be analyzed with a binary event tree. However, building a DET requires deterministic thermal-hydraulic (TH) simulation results for each branch condition, and as the number of possible combinations of operating safety-system trains grows, so does the number of TH simulation scenarios needed.

In the past, the number of TH simulation scenarios that could be run in a PSA study was itself limited by the available computing resources. As computing performance continues to improve, however, it is becoming increasingly possible to run large numbers of scenarios, and the environment needed to obtain the large volume of simulation results required by DET-based analysis is gradually becoming available [5]. As a result, the amount of output data to be analyzed also grows rapidly in proportion to the number of scenarios.

The key variables to be extracted are the reactor trip (RT) timing, the operating status of safety systems, and the peak cladding temperature (PCT), where PCT serves as a surrogate indicator for core damage determination based on the NRC 10 CFR 50.46 criterion (1477 K) [2]. In current PSA practice, manual review of individual output files must be carried out directly by expert personnel who can interpret TH code output and judge the behavior of safety systems. This is not a simple repetitive task but requires a deep understanding of thermal-hydraulic and system characteristics, so the time commitment of senior experts increases linearly with the number of scenarios. When the number of scenarios reaches the hundreds, this burden becomes even more severe, and as the data volume grows, the probability of fatigue-induced errors rises, making it difficult to maintain consistent judgments across scenarios [5]. Consequently, as the scale of simulation grows, the time commitment of expert personnel — i.e., the human cost — increases as well, directly driving up the cost of performing PSA. Extracting these variables consistently and accurately across hundreds of scenarios is a prerequisite for all subsequent PSA analysis, but manual processing is too costly to be sustainable, making an automated post-processing framework essential.

Against this background, this study presents an integrated post-processing tool. The tool automatically processes MARS-KS output (Excel) and produces an interactive Excel dashboard, allowing analysts to immediately compare PCT distributions by the number of operating safety-system trains without running additional simulations. This constitutes the first stage of a pipeline that proceeds from TH simulation output, through DET-structure visualization, to the automatic generation of PSA-software-compatible event trees.
...
[MARS-KS Output Excel] → [Automatic Variable Extraction] → [Interactive Excel Dashboard]
      (Input)                  (psa_analyzer)              (PSA Analyst Interface)
```
...
The interactive Excel dashboard is the tool's core output and serves as the interface that PSA analysts use directly, without any additional coding. It consists of an independent sheet for each accident type, with the following main features.
